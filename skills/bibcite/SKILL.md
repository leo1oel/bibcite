...
If `bibcite` is not on `PATH`, replace it in any command below with `uvx --from bibcite-cli bibcite`.
...
For any other result with `tidied` set to `false`, run `bibcite tidy <file>` once and inspect its exit code.
If formatting still fails, report that `bibtex-tidy` or `npx` is unavailable instead of formatting the file by hand.
